      ...
      if (siblingProducts.length > 0) {
        ...
        let siblingProductsHtml = siblingProducts.map((product) => {
          ...
          let colorHtml = product.colors && product.colors.length > 0
            ? `<ul class="colors-holder d-flex-r-c-c"> ${product.colors.slice(0, 5).map((proColor) => {
              let backgroundStyle = '';
        ...
        console.log('No sibling products found.');
      }
...
if(document.querySelector('.product-item')){
  const colorDots = document.querySelectorAll('.product-item .color-dot');
...
function scrollSlider5Items(options) {
  ...
  function setResponsive() {
    const responsiveSettings = [
      ...
      { breakpoint: 1550, settings: { slidesToShow: 6, slidesToScroll: 6 } },
      { breakpoint: 1800, settings: { slidesToShow: 7, slidesToScroll: 7 } }
    ];
